...
def MIcomparisonAll(transcriptionFactor1, transcriptionFactor2, method="svm", start=0, pieces=1):
    ...
    for i in range(start,40):
        ...
        print(params)
        k_rates = crmRates(a,b,c,d)
        testScores[i] = simMIall(k_rates, ts, x0, m, origin, crmPropensities, crmNmatrix, sampleTimes, method)
        print(testScores[i])
        np.save(fileName,testScores)
...
def fn():
    particleSwarmStartCRall("dot","mig1",np.zeros(10))
    particleSwarmContinueCRall("dot","mig1",8)