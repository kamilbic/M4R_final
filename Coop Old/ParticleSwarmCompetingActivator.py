...
def MIcomparisonAll(transcriptionFactor1, transcriptionFactor2, method="svm", start=0, pieces=1):
    ...
    for i in range(start,40):
        ...
        print(params)
        k_rates = camRates(a,b,c,d)
        testScores[i] = simMIall(k_rates, ts, x0, m, origin, camPropensities, camNmatrix, sampleTimes, method)
        print(testScores[i])
        np.save(fileName,testScores)
...
def fn():
    particleSwarmStartCAall("dot","mig1",np.zeros(10))
    particleSwarmContinueCAall("dot","mig1",8)