...
def simMIallDouble(k_rates, ts, x0, m, origin, Propensities, Nmatrix, sTimes, method='svm'):
    tstart = time.time()
...
def particleSwarmStartSimpleMixed(tFac1, tFac2, progress, pieces=1):
    ...
    startParticleSwarm('SimpleModelMixed', lattice, [[1,800],[1,800]],
                       4000, simMIallDouble, 10, ts, x0, m, origin, smPropensitiesDoubleCoop, smNmatrix,
                       sampleTimes, pieces, progress)

def particleSwarmContinueSimpleMixed(tFac1,tFac2,maxits, pieces=1):
    tf1,origin,sampleTimes = scaleTSall(tFac1)
    tf2,origin,sampleTimes = scaleTSall(tFac2)   
    ...
    progress = np.load("progressSimpleModelMixed.npy")
    lattice = np.load("latticeSimpleModel.npy")
    bestPos = np.load("bestPosSimpleModelMixed.npy")
    bestVals = np.load("bestValsSimpleModelMixed.npy")
    swarmPos = np.load("swarmPosSimpleModelMixed.npy")
    swarmVal = 1*np.load("swarmValsSimpleModelMixed.npy")
    currentPos = np.load("currentPosSimpleModelMixed.npy")
    currentVals = np.load("currentValsSimpleModelMixed.npy")
    velocities = np.load("velocitiesSimpleModelMixed.npy")
    parDict = json.load(open("parDictSimpleModelMixed.json"))
    
    modelName = 'SimpleModelMixed'
    
    continueParticleSwarm(lattice, parDict, currentPos, currentVals, bestPos,
                              bestVals, swarmPos, swarmVal, velocities, progress,
                              modelName, simMIallDouble, maxits, ts, x0, m, origin, 
                              smPropensitiesDoubleCoop, smNmatrix, sampleTimes, pieces)
    
def fn():
    particleSwarmStartSimpleMixed("dot6","maf1",np.zeros(10))
    particleSwarmContinueSimpleMixed("dot6","maf1",8)