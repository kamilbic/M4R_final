...
from CooperatingDerepressionModelFunctions import *
...
def simMI(k_rates, ts, x0, m, origin, Propensities, Nmatrix, sampleTimes, pieces, method):
    ...
    statesres = 0       
    k_rates = cdmRates(k_rates[0],k_rates[1],k_rates[2],k_rates[3])
    for i in range(m):
...
def MIcomparison(transcriptionFactor1, transcriptionFactor2, method="svm", start=0, pieces=1):
    ...
    fileName = transcriptionFactor1 + transcriptionFactor2 + "score" + "CoopDerepression" + method
    ...
    for i in range(start,40):
        ...
        d = params[3]
        print(params)
        k_rates = cdmRates(a,b,c,d)
        testScores[i] = simMI(k_rates, ts, x0, m, origin, cdmPropensities, cdmNmatrix, sampleTimes, pieces, method)
        print(testScores[i])
...
def timeComparison(transfact1,transfact2,pieces=1):
    ...
    for i in range(7):
        params = paramlist[i]
        times[i] = extrandeTimes(params, ts, x0, m, origin, cdmPropensities, cdmNmatrix, 
                     sampleTimes)
        print(times[i])
...
def particleSwarmStartD(transcriptionFactor1, transcriptionFactor2, progress, pieces=1):
    ...
    startParticleSwarm('CoopDerepression', lattice, [[1,1000],[1,1000],[1,1000],[1,1000]],10000,
                       simMI,15,ts, x0, m, origin, cdmPropensities, cdmNmatrix,
...
def particleSwarmContinueD(transcriptionFactor1, transcriptionFactor2, pieces, maxits):
    ...
    progress = np.load("progressCoopDerepression.npy")
    lattice = np.load("latticeDerepression.npy")
    bestPos = np.load("bestPosCoopDerepression.npy")
    bestVals = np.load("bestValsCoopDerepression.npy")
    swarmPos = np.load("swarmPosCoopDerepression.npy")
    swarmVal = 1*np.load("swarmValsCoopDerepression.npy")
    currentPos = np.load("currentPosCoopDerepression.npy")
    currentVals = np.load("currentValsCoopDerepression.npy")
    velocities = np.load("velocitiesCoopDerepression.npy")
    parDict = json.load(open("parDictCoopDerepression.json"))
    
    modelName = 'CoopDerepression'
    ...
    continueParticleSwarm(lattice, parDict, currentPos, currentVals, bestPos,
                              ...
                              cdmPropensities, cdmNmatrix, sampleTimes, pieces)
...
def MIcomparisonAll(transcriptionFactor1, transcriptionFactor2, method="svm", start=0, pieces=1):
    ...
    fileName = transcriptionFactor1 + transcriptionFactor2 + "all" + "CoopDerepression" + method
    ...
    for i in range(start,40):
        ...
        print(params)
        k_rates = cdmRates(a,b,c,d)
        testScores[i] = simMIall(k_rates, ts, x0, m, origin, cdmPropensities, cdmNmatrix, sampleTimes, method)
        print(testScores[i])
        np.save(fileName,testScores)
...
def particleSwarmStartCoopDall(transcriptionFactor1, transcriptionFactor2, progress, pieces=1):
    tf1,origin,sampleTimes = scaleTSall(transcriptionFactor1)    
    tf2,origin,sampleTimes = scaleTSall(transcriptionFactor2)
    ...
    startParticleSwarm('CoopDerepression', lattice, [[1,800],[1,800],[1,800],[1,800]],
                       10000, simMIall, 10, ts, x0, m, origin, cdmPropensities, cdmNmatrix,
                       sampleTimes, pieces, progress)

def particleSwarmContinueCoopDall(transcriptionFactor1, transcriptionFactor2,maxits, pieces=1):
    tf1,origin,sampleTimes = scaleTSall(transcriptionFactor1)    
    tf2,origin,sampleTimes = scaleTSall(transcriptionFactor2)
...
def fn():
    prog = np.zeros(10)
    particleSwarmStartCoopDall("dot6","dot6",prog)
    particleSwarmContinueCoopDall("dot6","dot6",maxits=8)