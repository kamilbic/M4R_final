...
def optimalBs(tF,l,h,k):
    ...
    fileName = tF + "ExCyc" + str(l) + "and" + str(h)
...
def particleSwarmStartExCall(transcriptionFactor1, transcriptionFactor2, progress, pieces=1):
    ...
    startParticleSwarm('ExCycling', lattice, [[1,800],[1,800],[1,800],[1,800]],
...
def particleSwarmContinueExCall(transcriptionFactor1, transcriptionFactor2,maxits, pieces=1):
    ...
    progress = np.load("progressExCycling.npy")
    lattice = np.load("latticeExCycling.npy")
    bestPos = np.load("bestPosExCycling.npy")
    bestVals = np.load("bestValsExCycling.npy")
    swarmPos = np.load("swarmPosExCycling.npy")
    swarmVal = 1*np.load("swarmValsExCycling.npy")
    currentPos = np.load("currentPosExCycling.npy")
    currentVals = np.load("currentValsExCycling.npy")
    velocities = np.load("velocitiesExCycling.npy")
    parDict = json.load(open("parDictExCycling.json"))
...
def fn():
    prog = np.load("progressExCycling.npy")
    particleSwarmStartExCall("dot6","dot6",prog)
    particleSwarmContinueExCall("dot6","dot6",maxits=8)
