...
def particleSwarmStartACall(transcriptionFactor1, transcriptionFactor2, progress, pieces=1):
    ...
    startParticleSwarm('ActiveCycling', lattice, [[1,800],[1,800],[1,800]],
                       10000, simMIall, 10, ts, x0, m, origin, activeCyclePropensities, activeCycleNmatrix,
...
def particleSwarmContinueACall(transcriptionFactor1, transcriptionFactor2,maxits, pieces=1):
    ...
    progress = np.load("progressActiveCycling.npy")
    lattice = np.load("latticeActiveCycling.npy")
    bestPos = np.load("bestPosActiveCycling.npy")
    bestVals = np.load("bestValsActiveCycling.npy")
    swarmPos = np.load("swarmPosActiveCycling.npy")
    swarmVal = 1*np.load("swarmValsActiveCycling.npy")
    currentPos = np.load("currentPosActiveCycling.npy")
    currentVals = np.load("currentValsActiveCycling.npy")
    velocities = np.load("velocitiesActiveCycling.npy")
    parDict = json.load(open("parDictActiveCycling.json"))
    
    modelName = 'ActiveCycling'
...
def fn():
    prog = np.load("progressActiveCycling.npy")
    particleSwarmStartACall("dot6","dot6",prog)
    particleSwarmContinueACall("dot6","dot6",maxits=8)