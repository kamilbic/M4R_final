...
def MIcomparisonAll(transcriptionFactor1, transcriptionFactor2, method="svm", start=0, pieces=1):
    ...
    if start == 0:
        ...
        if start < 10:
            ...
            start = sum(testScores[30:]>0)[0] + 30
            end = 40
    ...
    for i in range(start,end):
        ...
        print(params)
        k_rates = dmRates(a,b,c,d)
        miscore = simMIall(k_rates, ts, x0, m, origin, dmPropensities, dmNmatrix, sampleTimes, method)
        testScores = np.load(fileName+".npy")
...
def particleSwarmStartDall(transcriptionFactor1, transcriptionFactor2, progress, pieces=1):
    ...
    startParticleSwarm('DerepressionMixed', lattice, [[1,800],[1,800],[1,800],[1,800]],
...
def particleSwarmContinueDall(transcriptionFactor1, transcriptionFactor2,maxits, pieces=1):
    ...
    progress = np.load("progressDerepressionMixed.npy")
    lattice = np.load("latticeCompetingActivator.npy")
    bestPos = np.load("bestPosDerepressionMixed.npy")
    bestVals = np.load("bestValsDerepressionMixed.npy")
    swarmPos = np.load("swarmPosDerepressionMixed.npy")
    swarmVal = 1*np.load("swarmValsDerepressionMixed.npy")
    currentPos = np.load("currentPosDerepressionMixed.npy")
    currentVals = np.load("currentValsDerepressionMixed.npy")
    velocities = np.load("velocitiesDerepressionMixed.npy")
    parDict = json.load(open("parDictDerepressionMixed.json"))
    
    modelName = 'DerepressionMixed'
...
def fn():
    prog = np.load("progressDerepressionMixed.npy")
    particleSwarmStartDall("dot6","maf1",prog)
    particleSwarmContinueDall("dot6","maf1",8)