...
from DataProcess import scaleTS2st
from DataProcess import scaleTSall
from MIdecodingSimp import estimateMIS
from CompetingActivatorModelFunctions import *
...
def simMI(k_rates, ts, x0, m, origin, Propensities, Nmatrix, sampleTimes, pieces, method):
    ...
    statesres = 0       
    k_rates = camRates(k_rates[0],k_rates[1],k_rates[2],k_rates[3])
    for i in range(m):
        ...
        resultsCol[i,:] = collapsepl(np.reshape(results[i,:],(2,-1)), pieces)
...
def MIcomparison(transcriptionFactor1, transcriptionFactor2, method="svm", start=0, pieces=1):
    ...
    fileName = transcriptionFactor1 + transcriptionFactor2 + "score" + "CompetingActivator" + method
    ...
    for i in range(start,40):
        ...
        d = params[3]
        print(params)
        k_rates = camRates(a,b,c,d)
        testScores[i] = simMI(k_rates, ts, x0, m, origin, camPropensities, camNmatrix, sampleTimes, pieces, method)
        print(testScores[i])
...
def particleSwarmStartCA(transcriptionFactor1, transcriptionFactor2, progress, pieces=1):
    tf1,origin,sampleTimes = scaleTS(transcriptionFactor1)    
    tf2,origin,sampleTimes = scaleTS(transcriptionFactor2)
    ...
    startParticleSwarm('CompetingActivator', lattice, [[1,1000],[1,1000],[1,1000],[1,1000]],
                       10000, simMI, 15, ts, x0, m, origin, camPropensities, camNmatrix,
                       sampleTimes, pieces, progress)

def particleSwarmContinueCA(transcriptionFactor1, transcriptionFactor2, pieces, maxits):
    tf1,origin,sampleTimes = scaleTS(transcriptionFactor1)    
    tf2,origin,sampleTimes = scaleTS(transcriptionFactor2)
    ...
    progress = np.load("progressCompetingActivator.npy")
    lattice = np.load("latticeCompetingActivator.npy")
    bestPos = np.load("bestPosCompetingActivator.npy")
    bestVals = np.load("bestValsCompetingActivator.npy")
    swarmPos = np.load("swarmPosCompetingActivator.npy")
    swarmVal = 1*np.load("swarmValsCompetingActivator.npy")
    currentPos = np.load("currentPosCompetingActivator.npy")
    currentVals = np.load("currentValsCompetingActivator.npy")
    velocities = np.load("velocitiesCompetingActivator.npy")
    parDict = json.load(open("parDictCompetingActivator.json"))
    
    modelName = 'CompetingActivator'
    ...
    continueParticleSwarm(lattice, parDict, currentPos, currentVals, bestPos,
                              ...
                              camPropensities, camNmatrix, sampleTimes, pieces)
...
def MIcomparisonAll(transcriptionFactor1, transcriptionFactor2, method="svm", start=0, pieces=1):
    ...
    fileName = transcriptionFactor1 + transcriptionFactor2 + "all" + "CompetingActivator" + method
    ...
    for i in range(start,end):
        ...
        print(params)
        k_rates = camRates(a,b,c,d)
        miscore = simMIall(k_rates, ts, x0, m, origin, camPropensities, camNmatrix, sampleTimes, method)
        testScores = np.load(fileName+".npy")
...
def particleSwarmStartCAall(transcriptionFactor1, transcriptionFactor2, progress, pieces=1):
    tf1,origin,sampleTimes = scaleTSall(transcriptionFactor1)    
    tf2,origin,sampleTimes = scaleTSall(transcriptionFactor2)
    ...
    startParticleSwarm('CompetingActivatorMixed', lattice, [[1,800],[1,800],[1,800],[1,800]],
                       10000, simMIall, 10, ts, x0, m, origin, camPropensities, camNmatrix,
                       sampleTimes, pieces, progress)

def particleSwarmContinueCAall(transcriptionFactor1, transcriptionFactor2,maxits, pieces=1):
    tf1,origin,sampleTimes = scaleTSall(transcriptionFactor1)    
    tf2,origin,sampleTimes = scaleTSall(transcriptionFactor2)
    ...
    progress = np.load("progressCompetingActivatorMixed.npy")
    lattice = np.load("latticeCompetingActivator.npy")
    bestPos = np.load("bestPosCompetingActivatorMixed.npy")
    bestVals = np.load("bestValsCompetingActivatorMixed.npy")
    swarmPos = np.load("swarmPosCompetingActivatorMixed.npy")
    swarmVal = 1*np.load("swarmValsCompetingActivatorMixed.npy")
    currentPos = np.load("currentPosCompetingActivatorMixed.npy")
    currentVals = np.load("currentValsCompetingActivatorMixed.npy")
    velocities = np.load("velocitiesCompetingActivatorMixed.npy")
    parDict = json.load(open("parDictCompetingActivatorMixed.json"))
    
    modelName = 'CompetingActivatorMixed'
...
def fn():
    prog = np.load("progressCompetingActivatorMixed.npy")
    particleSwarmStartCAall("dot6","maf1",prog)
    particleSwarmContinueCAall("dot6","maf1",8)