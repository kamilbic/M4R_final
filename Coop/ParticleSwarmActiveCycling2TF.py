...
def simMI(k_rates, ts, x0, m, origin, Propensities, Nmatrix, sampleTimes, pieces, method):
    ...
    plt.figure()
    plt.plot(statesres[0][origin-20:origin+20],label='I')
    plt.plot(statesres[1][origin-20:origin+20],label='A')
    plt.plot(statesres[2][origin-20:origin+20],label='B')
    plt.plot(statesres[1][origin-20:origin+20]+statesres[2][origin-20:origin+20],label='A+B')
...
def MIcomparisonAll(transcriptionFactor1, transcriptionFactor2, method="svm", start=0, pieces=1):
    ...
    for i in range(start,end):
        ...
        print(params)
        k_rates = activeCycleRates(a,b,c)
        miscore = simMIall(k_rates, ts, x0, m, origin, activeCyclePropensities, activeCycleNmatrix, sampleTimes, method)
        testScores = np.load(fileName+".npy")
...
def fn():
    particleSwarmStartACall("dot6","maf1",np.zeros(10))
    particleSwarmContinueACall("dot6","maf1",8)