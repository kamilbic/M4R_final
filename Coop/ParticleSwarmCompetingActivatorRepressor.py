...
from CompetingActivatorRepressorModelFunctions import *
...
def MIcomparison(transcriptionFactor1, transcriptionFactor2,method="svm",start=0,pieces=1):
    ...
    fileName = transcriptionFactor1 + transcriptionFactor2 + "score" + "CompetingActivatorRepressor" + method
...
def MIcomparisonAll(transcriptionFactor1, transcriptionFactor2, method="svm", start=0, pieces=1):
    ...
    fileName = transcriptionFactor1 + transcriptionFactor2 + "all" + "CompetingActivatorRepressor" + method
    ...
    for i in range(start,end):
        ...
        print(params)
        k_rates = carmRates(a,b,c,d)
        miscore = simMIall(k_rates, ts, x0, m, origin, carmPropensities, carmNmatrix, sampleTimes, method)
        testScores = np.load(fileName+".npy")
...
def particleSwarmStartCARall(transcriptionFactor1, transcriptionFactor2, progress, pieces=1):
    ...
    startParticleSwarm('CompetingActivatorRepressorMixed', lattice, [[1,800],[1,800],[1,800],[1,800]],
...
def particleSwarmContinueCARall(transcriptionFactor1, transcriptionFactor2,maxits, pieces=1):
    ...
    lattice = np.load("latticeCompetingActivator.npy") 
    progress = np.load("progressCompetingActivatorRepressorMixed.npy")
    bestPos = np.load("bestPosCompetingActivatorRepressorMixed.npy")
    bestVals = np.load("bestValsCompetingActivatorRepressorMixed.npy")
    swarmPos = np.load("swarmPosCompetingActivatorRepressorMixed.npy")
    swarmVal = 1*np.load("swarmValsCompetingActivatorRepressorMixed.npy")
    currentPos = np.load("currentPosCompetingActivatorRepressorMixed.npy")
    currentVals = np.load("currentValsCompetingActivatorRepressorMixed.npy")
    velocities = np.load("velocitiesCompetingActivatorRepressorMixed.npy")
    parDict = json.load(open("parDictCompetingActivatorRepressorMixed.json"))
    
    modelName = 'CompetingActivatorRepressorMixed'
...
def fn():
    prog = np.load("progressCompetingActivatorRepressorMixed.npy")
    particleSwarmStartCARall("dot6","maf1",prog)
    particleSwarmContinueCARall("dot6","maf1",8)